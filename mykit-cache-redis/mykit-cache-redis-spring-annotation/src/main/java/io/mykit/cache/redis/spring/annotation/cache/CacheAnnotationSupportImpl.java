...
@Slf4j
@Component
public class CacheAnnotationSupportImpl implements CacheAnnotationSupport {
    ...
    public void registerInvocation(Object targetBean, Method targetMethod, Class[] invocationParamTypes,
       ...
       try{
           ...
           final CachedAnnotationMethodInvocation invocation = new CachedAnnotationMethodInvocation(key, targetBean, targetMethod, invocationParamTypes, invocationArgs);
           for (Cache cache : caches) {
               if (cache instanceof CustomizedAnnotationRedisCache) {
                   CustomizedAnnotationRedisCache redisCache = ((CustomizedAnnotationRedisCache) cache);
                   // 将方法信息放到redis缓存
                   //RedisTemplate redisTemplate = RedisTemplateUtils.getRedisTemplate(redisConnectionFactory);
                   String invocationCacheKey = CacheAnnotationSupportUtils.getInvocationCacheKey(redisCache.getCacheKey(key));
    ...
    public void refreshCacheByKey(String cacheName, String cacheKey) {
        //RedisTemplate redisTemplate = RedisTemplateUtils.getRedisTemplate(redisConnectionFactory);
        //在redis拿到方法信息，然后刷新缓存
        String invocationCacheKey = CacheAnnotationSupportUtils.getInvocationCacheKey(cacheKey);
        log.debug("refreshCacheByKey==>>" + invocationCacheKey);
        Object result = redisTemplate.opsForValue().get(invocationCacheKey);

        if (result != null && result instanceof CachedAnnotationMethodInvocation) {
            CachedAnnotationMethodInvocation invocation = (CachedAnnotationMethodInvocation) result;
            // 执行刷新方法
    ...
    @Override
    public Map<String, CacheAnnotationTime> getCacheTimes(String cacheName) {
        log.debug("getCacheTimes获取到的cacheName参数===>>>" + cacheName);
        ConcurrentMap<String, CacheAnnotationTime> map = new ConcurrentHashMap<>();
        //默认超时时间5分钟
        long expireTime = CacheAnnotationConstants.DEFAULT_EXPIRATION_SECOND_TIME;
        //默认刷新缓存时间为280
        long preloadTime = CacheAnnotationConstants.DEFAULT_PRELOAD_SECOND_TIME;
        //默认的缓存Key
        String cacheKey = CacheAnnotationConstants.DEFAULT_EXPIRATION_KEY;
        //传递的参数不为空
        if(!StringUtils.isEmpty(cacheName)){
            if (cacheName.contains(CacheAnnotationConstants.SEPARATOR)){
                String[] cacheArray = cacheName.split(CacheAnnotationConstants.SEPARATOR);
                ...
                if(cacheArray.length > 1){
                    try{
                        ...
                        expireTime = CacheAnnotationConstants.DEFAULT_EXPIRATION_SECOND_TIME;
                    }
                }
                ...
                if(cacheArray.length > 2){
                    try{
                        ...
                        preloadTime = CacheAnnotationConstants.DEFAULT_PRELOAD_SECOND_TIME;
                    }
                }
            }else{
        ...
        map.put(cacheKey, new CacheAnnotationTime(expireTime, preloadTime));
        return map;
    }
    ...
    private void refreshCache(CachedAnnotationMethodInvocation invocation, String cacheName) {
        try {
            // 通过代理调用方法，并记录返回值
            ...
            redisTemplate.expire(CacheAnnotationSupportUtils.getInvocationCacheKey(redisCache.getCacheKey(invocation.getKey())), expireTime, TimeUnit.SECONDS);
    ...
    private Object invoke(CachedAnnotationMethodInvocation invocation) throws Exception {
        ...
        //获取当前类的接口的Class对象
        Class<?> clazz = ReflectionAnnotationUtils.getInterfaceClass(invocation.getTargetBean(), invocation.getTargetMethod(), invocation.getParameterTypes());
        log.debug(CacheAnnotationSupportImpl.class.getName() + "类加载的路径：" + clazz.getResource("/").getPath()+ ", hashcode:" + Hashing.MURMUR_HASH.hash(clazz.getResource("/").getPath()));
        String contextKey = SpringAnnotationContextWrapper.getContextKey(clazz);
        log.debug("contextKey===>>> " + contextKey);
        log.debug(CacheAnnotationSupportImpl.class.getName() + " applicationContext===>>>" + SpringAnnotationContextWrapper.getApplicationContext(contextKey));
        Object bean = SpringAnnotationContextWrapper.getBean(contextKey, clazz);
        ...
        if (clazz.isInterface()){
            ...
            target = ReflectionAnnotationUtils.getTarget(bean);
        }
    ...
    private Collection<String> generateValue(Set<String> annotatedCacheNames) {
        ...
        for (final String cacheName : annotatedCacheNames) {
            String[] cacheParams = cacheName.split(CacheAnnotationConstants.SEPARATOR);
            // 截取名称获取真实的value值
